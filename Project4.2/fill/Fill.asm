...
M=-1 // sets value in current address to -1, so that the whole word 1...1 (16 bits long), meaning - all 16 pixels will be "painted" black.
...
@i
A=D+M // adds the current index to the screen's first address in order to color the current set of 16 pixels
M=0 // sets value in current address to 0, so that the whole word will be 0....0 (16 bits long), meaning - all 16 pixels will be "painted" white.
@LOOP // jumps back to indexer in order to progress the index backwards.
0;JMP